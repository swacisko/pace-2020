...
DepthTree DepthTreePivotMaker::makeHallSetPivots(DepthTree &dt, double balance, bool useDeepestPath) {
    ...
    if(debug){ DEBUG(T0Nodes); DEBUG(P); }
    ...
    if(debug){ DEBUG(P2); DEBUG(neighP2); DEBUG(PNoP2); }
...
DepthTree DepthTreePivotMaker::makeHallSetPivotsSinglePass(DepthTree &dt, bool useDeepestPath) {
    ...
    if(debug) for( int i=0; i<T; i++ ) cerr << i << ": " << tree[i] << "\t" << data[i] << endl;
    ...
    const function<void()> writeState = [&](){
...
DepthTree DepthTreePivotMaker::makeAllPivots(DepthTree &dt) {
    ...
    if ( cnf.pivots_to_use_mask & (1<<Pivots::BlockPivots) ) {
    ...
    if(cnf.pivots_to_use_mask & (1<<Pivots::HallSetPivots)) {
