...
VI MaxMatchBipartite::getMaximalMatchingRandom(VVI & G, VB& bipartition){
    VPII edges = GraphUtils::getGraphEdges(G);
    random_shuffle( edges.begin(), edges.end() );
    ...
    for( PII e : edges ){
        ...
        if( bipartition[a] == bipartition[b] ) continue;
...
VI MaxMatchBipartite::getMaximumMatchingInBipartition(VVI &G, VB &bipartition, VI & initialMatching, bool fastSearch ){
    VI matching = initialMatching;
...
void MaxMatchBipartite::applyAlternatingPath(VI& matching, VI & path){
    if( path.size() %2 == 0 ){
        applyAugmentingPath( matching, path );
    }else{
        if( path.size() <= 3 ) return;
        if( matching[ path[0] ] != path[1] ) reverse( path.begin(), path.end() );
        matching[ path[0] ] = -1;
...
VI MaxMatchBipartite::getRandomMinimalHallViolator(VVI &V, VB &bipartition, VI &matching) {
    VI res;
    VB was(V.size(),false);
    VI neigh;
    ...
    for( int i=0; i<neigh.size(); i++ ){
        ...
        if( bipartition[p] == false ){
            ...
            if( matching[p] == -1 ){
                cerr << "ERROR, unmatched node " << p << ", augmenting path exists and hall violator may not even exist! " << endl;exit(1);
            }else{
                int d = matching[p];
                neigh.push_back(d);
                was[d] = true;
